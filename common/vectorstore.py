...
і текст для цитування. Ім'я колекції каже поле `collection` у config.json
примірника (`docs-ecmascript`); без поля воно виводиться з імені теки примірника.
Одна колекція — один корпус і одна модель: інша модель дає вектори іншої довжини
й геометрії, рахувати відстань між непорівнюваними числами не можна, тож
розбіжність довжини ensure_collection зупиняє вголос, а не виправляє мовчки.
...
# Раніше ім'я сюди потрапляло лише через export у df, і сервер, піднятий повз
# df (напряму чи Inspector'ом — DF_INSTANCE_DIR там передають, QDRANT_COLLECTION
# ні), шукав у колекції spec-suite-bge-small, якої ніхто не заливав. Тепер поле
# читає сам Python, і всі способи запуску дивляться в одну колекцію. Умовчання
# без поля — з імені теки примірника, а не зі сталої модуля: другий домен з
# порожньою конфігурацією отримає власну колекцію, а не колекцію першого.
# root() тут лінивий (третя гілка or): без DF_INSTANCE_DIR він падає з
# підказкою — вгадувати колекцію без примірника не можна так само, як корпус.
COLLECTION = (os.getenv("QDRANT_COLLECTION")
              or instance.config().get("collection")
              or f"docs-{instance.root().name}")
